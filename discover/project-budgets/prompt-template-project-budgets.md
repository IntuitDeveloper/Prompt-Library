...
**Capability Check:** Before making any Projects or Budget API calls, verify the connected QuickBooks company meets the three company-level prerequisites below. Run these checks in order and stop at the first failure. (Check 4 below is a separate, per-project check that cannot be verified ahead of time — see its section for how it's actually handled, at budget-create time in Task 2.)
...
#### Check 4 — Project cost method (per-project, verified at create time)
A budget can only be created for a project whose cost method is NOT `Basic` (i.e. the project uses the Advanced cost method). **`ProjectManagement_Project` has no `costMethod` (or similarly named) field in the GraphQL schema — do NOT query one, and do NOT invent a field name to pre-check this.** There is no reliable way to verify cost method ahead of time via the Projects GraphQL API. Instead, attempt Task 2's create call directly; if the project uses Basic costing, the server itself rejects it (HTTP 200) with `errorCode: "PNB-INPUT-87"` / `PROJECT_COST_METHOD_IS_BASIC`. Catch that specific error code and surface a user-friendly message:
> "Project budgets are not supported for projects with Basic cost method. Switch this project to Detailed/Advanced job costing in QuickBooks, or select a different project."
...
**A project can have at most one budget (1:1).** Attempting to create a second budget for a project that already has one fails with `errorCode: "PNB-INPUT-72"` / `BUDGET_EXIST_WITH_SAME_LINKED_ENTITY_ID` — this is an EXPECTED, recoverable condition, not a fatal error. Handle it gracefully rather than crashing:
1. Attempt the create for the currently selected project.
2. If it fails with `PNB-INPUT-72`, log that this project already has a budget and try the NEXT project from Task 1 Step A's discovered list (do not retry the same project).
3. Repeat for each remaining discovered project until one create succeeds.
4. If every discovered project already has a budget (or Step A returned zero projects), fall back to creating a new project via Step B, then create the budget for that new project.
...
- **`sequenceId` and `order` are STRINGS, not numbers — verified live.** Sending them as JSON numbers (e.g. `"order": 1`) fails GraphQL validation. Always send them as string literals (e.g. `"order": "1"`), on create, update, and when reading them back.
...
- **`budgetDetails` array position does NOT reflect submission or line order — verified live.** The server does not preserve the order lines were submitted in; the array can come back in a different sequence than sent (confirmed: submitting order `"1"` then `"2"` returned `"2"` first, `"1"` second, on the same request). Before displaying, ALWAYS sort the line list by the numeric value of its `order` field — never rely on array index for display sequence.
...
- **Delete a line:** re-send the line's `sequenceId`, `type`, and `itemId` (the same `itemId` it was created with — read it back from Task 3) alongside **`deleted: true`**. **A delete-marked line sent as only `{ sequenceId, deleted: true }` (missing `type` and/or `itemId`) is REJECTED — verified live** with either `errorCode: "PNB-INPUT-74"` or `"Invalid item id entered, product and services id can't be blank"`, depending on which field is missing. Do NOT assume the server ignores other fields on a delete just because it's being removed — it still validates them as if this were a normal line. Minimal known-working delete shape: `{ "sequenceId": "<id>", "type": "ITEM", "itemId": "<the line's original itemId>", "deleted": true }`.
...
- **UI (REQUIRED — not optional):** In addition to the backend integration logic, build a minimal local web UI so a developer can exercise this workflow without a REST client: a page served locally with a form to select/discover a Project, enter budget line items (item, unit cost, quantity), a "Create Budget" button that calls the create logic, and a results panel that renders the created budget (name, type, total, line items) using the read logic. Keep the UI plain HTML/CSS/vanilla JS (or the target language/framework's minimal built-in server) — no frontend build step, no external UI framework dependency. The UI calls the same backend functions used elsewhere in this workflow; do not duplicate API logic in the frontend. This UI must be built in every case — it is not conditional and there is no "skip the UI" option. **The UI is the program's only entry point.** Running the program (e.g. `gradle run`, `node index.js`, `python main.py`) with no arguments must start the local web server directly. Do NOT gate the UI behind a `--ui` flag, an environment variable, or any other opt-in condition, and do NOT also build a separate CLI/console workflow as an alternative or default mode — the UI *is* the runnable verification example this workflow produces.
...
13. **No `costMethod` field:** `ProjectManagement_Project` has no `costMethod` (or similarly named) field — do not query one to pre-check Basic vs. Advanced costing. That check is only observable by attempting the budget create and catching `PNB-INPUT-87` (see Check 4) — verified live.
14. **`PNB-INPUT-72` is recoverable, not fatal:** a project already having a budget (1:1 constraint) is an expected condition. Never let it crash the program or terminate the workflow — catch it specifically and try the next discovered project (or create a new one) per Task 2's retry flow.
15. **Never trust `budgetDetails` array order for display:** the response array order does not match submission order — verified live. Always sort by the `order` field before rendering line items; do not assume array index N corresponds to the Nth submitted line.
16. **If Blocked/Missing Info:** if required fields to compile a functional request are missing, STOP and state what's missing instead of guessing.
