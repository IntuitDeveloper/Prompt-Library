...
- **Endpoint:** `https://qb.api.intuit.com/graphql` or `https://qb-sandbox.api.intuit.com/graphql`. Refer to `https://developer.intuit.com/app/developer/gql/docs/api/qbexternal/queries/`
...
- **Query:** `query ActiveCustomDimensionDefinitionsQuery {
...
- **Empty State Handling:** If no dimensions, catch this state, provide an error ("No dimensions found. Please set them up in QuickBooks."), and provide a redirect link to: `https://qbo.intuit.com/app/class`.
...
- **Constraint:** The `valueId` used for `salesreceipt` tagging MUST come from the `appFoundationsActiveCustomDimensionValues` query response, not from the definitions query.
- Pass `definitionId` as the filter to fetch values; use the returned `id` as `valueId`.
- **Values Query** (use `definitionId` from definitions as a filter): `query CustomDimensionsValuesQuery {
...
- **Typing:** (If applicable) Provide `hints (dataclasses)` interfaces for the `CustomExtensions` and `Dimension` objects.
- **Output (integration mode: `new`):** Provide modular, clean code and a runnable verification example.
  - **If mode is `new`:** Create a self-contained project in a dedicated folder named `ies-dimensions-python` (no spaces). Include a `README.md` explaining how to run the code, a dependency manifest, and a brief architectural diagram showing the data flow from GraphQL to REST.
